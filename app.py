...
def _comprimir_imagen(datos, media_type):
    """
    Reduce el tamano de una imagen grande antes de mandarla a Claude.
    Las imagenes de mas de 1.5 MB se reescalan y reencodean como JPEG
    de calidad media, lo que baja muchisimo el pico de memoria sin
    perder legibilidad para OCR.
    Si Pillow no esta disponible o algo falla, devuelve los datos originales.
    """
    if len(datos) <= 1_500_000:
        return datos, media_type
    try:
        from PIL import Image
        img = Image.open(BytesIO(datos))
        # Reducir a 2000px de lado mayor (suficiente para OCR de un texto).
        img.thumbnail((2000, 2000), Image.LANCZOS)
        # Pasar a RGB si tiene canal alpha, para poder guardar como JPEG.
        if img.mode in ("RGBA", "P"):
            img = img.convert("RGB")
        salida = BytesIO()
        img.save(salida, format="JPEG", quality=80, optimize=True)
...
def _texto_de_pdf_o_imagen(datos, nombre):
    ...
    if es_pdf:
        ...
        # Reducir imagenes grandes antes de codificar.
        datos, media_type = _comprimir_imagen(datos, _IMG_MEDIA[ext])
        bloque = {
            "type": "image",
            "source": {
                "type": "base64",
    ...
    # Liberar los bytes crudos en cuanto pasamos al base64.
    datos = None
    ...
    try:
        ...
        # Liberar el bloque grande en cuanto Claude responde.
        bloque = None
