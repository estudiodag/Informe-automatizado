...
def _texto_de_excel(datos, nombre, max_filas=None):
    """
    ...
    Si se pasa max_filas, solo se incluyen las primeras N filas de cada
    hoja. Esto es util cuando la grilla de piezas se procesa por otro
    camino y a Claude solo le interesan los datos de cabecera.
    ...
    try:
        if es_xls_viejo:
            ...
            for sh in wb.sheets():
                ...
                limite = sh.nrows if max_filas is None else min(sh.nrows, max_filas)
                for r in range(limite):
                    ...
                    for c in range(sh.ncols):
                        ...
                        # tipo 3 = XL_CELL_DATE -> formatear DD/MM/AAAA
                        if tipo == 3:
                            try:
                                dt = xldate_as_datetime(v, wb.datemode)
                                v = f"{dt.day:02d}/{dt.month:02d}/{dt.year:04d}"
                            except Exception:
                                pass
            ...
            for ws in wb.worksheets:
                ...
                for fila in ws.iter_rows(max_row=max_filas):
                    celdas = []
                    for celda in fila:
                        v = celda.value
                        if v in (None, ""):
                            continue
                        # Fechas: formatear como DD/MM/AAAA
                        if isinstance(v, (_dt.date, _dt.datetime)):
                            v = f"{v.day:02d}/{v.month:02d}/{v.year:04d}"
                        celdas.append("%s=%s" % (celda.coordinate, v))
                    if celdas:
                        lineas.append(" | ".join(celdas))
        else:
...
def procesar_informe():
    ...
    try:
        ...
        # Bytes del Excel para el parser local de piezas.
        excel_bytes = None
        excel_nombre = ""
        ...
        if "excel_perit" in request.files:
            ...
            if f and f.filename != "":
                excel_bytes = f.read()
                excel_nombre = f.filename
                # Mandar a Claude SOLO las primeras filas del Excel
                # (datos del vehiculo, asegurado, fecha, taller). La
                # grilla de piezas la procesa el parser local en
                # procesador.py, no se manda a Claude. Esto evita el
                # timeout en peritajes con muchas piezas.
                from io import BytesIO as _IOBytes
                # Reset del puntero por si se necesita re-leer.
                class _FakeFile:
                    ...
                    def read(self):
                        return self._b
                try:
                    texto_excel_cabecera = _texto_de_excel(
                        excel_bytes, excel_nombre, max_filas=17)
                ...
                if texto_excel_cabecera:
                    bloque = ("\n\n=== DATOS DEL PERITAJE (cabecera del Excel) ===\n"
        ...
        #     este paso se elimino para ahorrar memoria) ---
        ...
        informe_bytes, data, _ = procesar(
            ...
            excel_peritacion_bytes=excel_bytes,
            excel_peritacion_nombre=excel_nombre,
            overrides=overrides,
        )
