...
def _mejorar_observaciones(texto):
    ...
    instruccion = (
        ...
        "seguros. Reescribi las siguientes observaciones de un perito en "
        "tono tecnico, formal y profesional, con oraciones completas y "
        "buena ortografia. REGLAS ESTRICTAS:\n"
        "- NO agregues informacion, conclusiones ni datos que no esten en "
        "el texto original.\n"
        ...
        "- Manteniendo todo lo anterior, mejora la redaccion.\n"
        "Devolve UNICAMENTE el texto reescrito, sin comillas ni "
        "comentarios."
    )
...
def parsear_texto(texto):
    ...
    # Nota: la reescritura automatica de observaciones se desactivo
    # porque agregaba una segunda llamada a Claude por informe, lo que
    # duplicaba el pico de memoria y causaba SIGKILL en el plan free de
    # Render (512 MB). La funcion _mejorar_observaciones queda definida
    # arriba por si se reactiva en un plan con mas RAM.
...
def completar_plantilla(plantilla_bytes, data):
    ...
    if len(hojas) > 2:
        ...
        # Si no hubo cotizacion de piezas pero el Excel del peritaje
        # traia una fila "REPUESTOS $X" ya consolidada, usar ese monto.
        if total_repuestos == 0 and data.get("_totalRepuestosExcel"):
            total_repuestos = data["_totalRepuestosExcel"]
...
def _enriquecer_desde_excel(data, texto_peritacion):
    ...
    # --- 3. Precios individuales por pieza ---
    # Si una pieza a CAMBIAR vino con precio 0, buscar en las filas del
    # Excel una que mencione el mismo nombre y tenga un monto. Usar el
    # monto mas grande de esa fila (descartando cantidades pequenas).
    def _normalizar(s):
        """Quita acentos, baja a minusculas y deja solo letras/digitos."""
        s = str(s).upper()
        repl = (("Á", "A"), ("É", "E"), ("Í", "I"), ("Ó", "O"),
                ("Ú", "U"), ("Ü", "U"), ("Ñ", "N"))
        for a, b in repl:
            s = s.replace(a, b)
    ...
    def _palabras_significativas(texto):
        """Devuelve las palabras 'fuertes' (>=3 letras, no genericas)."""
        stop = {"DE", "DEL", "LA", "EL", "CON", "PARA", "POR",
                "TD", "TI", "DER", "IZQ", "DERECHO", "IZQUIERDO",
                "DERECHA", "IZQUIERDA", "TRASERO", "TRASERA",
                "DELANTERO", "DELANTERA"}
        return [p for p in _normalizar(texto).split()
                if len(p) >= 3 and p not in stop]

    # Recorrer cada pieza del data que vino sin precio y buscar match.
    # Las filas del Excel se "consumen" al matchear, asi dos piezas con
    # el mismo nombre toman precios distintos en orden.
    filas_usadas = set()
    for pieza in data.get("danos", []):
        if pieza.get("accion") != "CAMBIAR":
            continue
        if pieza.get("precio"):  # ya tiene precio (cotizacion paso 3)
            continue
        nombre_pieza = pieza.get("pieza", "")
        palabras_pieza = _palabras_significativas(nombre_pieza)
        if not palabras_pieza:
            continue

        # Buscar la fila del Excel que mejor matchee con la pieza.
        mejor = None  # (score, monto, indice_fila)
        for idx, (etiquetas, montos) in enumerate(montos_por_fila):
            if idx in filas_usadas:
                continue
            # Saltar filas de totales/resumen/labels generales.
            if any(p in e for e in etiquetas
                   for p in ("TOTAL", "FRANQUICIA", "NETO", "RESUMEN",
                             "REPUESTO", "MANO DE OBRA", "PARTE",
                             "MOTOR", "TREN", "OTROS")):
                continue
            texto_fila = " ".join(etiquetas)
            score = sum(1 for p in palabras_pieza if p in texto_fila)
            if score == 0:
                continue
            montos_validos = [m for m in montos if m >= 100]
            if not montos_validos:
                continue
            monto = max(montos_validos)
            if mejor is None or score > mejor[0]:
                mejor = (score, monto, idx)

        if mejor:
            pieza["precio"] = mejor[1]
            filas_usadas.add(mejor[2])

    return data


def procesar(plantilla_bytes, texto_peritacion="", excel_peritacion_bytes=None,
             cotizar=False, overrides=None):
    """
    ...
    # Enriquecimiento sin llamadas extra: completa montos que Claude
    # puede no haber capturado bien, leyendo el texto del Excel.
    data = _enriquecer_desde_excel(data, texto_peritacion)
