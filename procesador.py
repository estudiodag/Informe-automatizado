...
def _enriquecer_desde_excel(data, texto_peritacion):
    ...
    def _a_numero(txt):
        # "$ 1.708.000,50" -> 1708000.5 ; "$1.000.000" -> 1000000 ;
        # "100000" -> 100000 ; "100,50" -> 100.5
        ...
        if "," in s:
            s = s.replace(".", "").replace(",", ".")
        # Sin coma: si hay 2+ puntos, todos son separadores de miles.
        # Si hay 1 solo punto seguido de 3 digitos exactos, tambien es
        # separador de miles (formato AR, ej "1.000").
        elif s.count(".") >= 2:
            s = s.replace(".", "")
        elif s.count(".") == 1:
    ...
    if total_piezas == 0:
        for etiquetas, montos in montos_por_fila:
            if any("REPUESTO" in e for e in etiquetas):
                total = max(montos)
                data["_totalRepuestosExcel"] = total

                # Distribuir el total en partes iguales entre las piezas
                # a CAMBIAR, asi cada fila de la Hoja 2 muestra un valor
                # y la suma cuadra con el Total Repuestos. El ultimo
                # ajuste de redondeo va a la primera pieza.
                piezas_cambiar = [d for d in data.get("danos", [])
                                  if d.get("accion") == "CAMBIAR"]
                if piezas_cambiar and total > 0:
                    parte = round(total / len(piezas_cambiar))
                    resto = total - parte * len(piezas_cambiar)
                    for i, d in enumerate(piezas_cambiar):
                        d["precio"] = parte + (resto if i == 0 else 0)
                break
