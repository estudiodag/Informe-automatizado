...
def _copiar_imagenes(ws_origen, ws_destino):
    ...
    IMPORTANTE: tras hacer deepcopy del anchor, hay que dejar
    anchor.pic = None. Sin esto, openpyxl reusa el mismo cNvPr id en
    el XML generado para ambas hojas, y Excel no renderiza las
    imagenes duplicadas. Asignar None hace que openpyxl regenere
    automaticamente un id unico al serializar.
    """
    try:
        ...
        for img in getattr(ws_origen, "_images", []):
            ...
            try:
                ...
                # Limpiar el pic interno para forzar regeneracion con
                # id unico al guardar el archivo.
...
def procesar(plantilla_bytes, texto_peritacion="", excel_peritacion_bytes=None,
    ...
    """
    # 1) El texto libre va a Claude (datos del vehiculo, asegurado,
    #    observaciones, fecha, taller, etc).
    data = parsear_texto(texto_peritacion) if texto_peritacion else data_vacia()

    # 2) Si vino un Excel, parsear localmente las piezas. Esto sobrescribe
    #    cualquier "danos" que Claude haya devuelto (que en este flujo
    #    será vacío porque la grilla ya no se le manda).
    if excel_peritacion_bytes:
        try:
            from parser_peritaje import parsear_peritaje_excel
            resultado = parsear_peritaje_excel(
                excel_peritacion_bytes, excel_peritacion_nombre or "x.xlsx")
            if resultado and resultado.get("danos"):
                data["danos"] = resultado["danos"]
            if resultado and resultado.get("manoObra"):
                # MERGE: solo escribimos las claves que devolvio el
                # parser, preservando las otras (los *Valor unitarios
                # que vienen de la plantilla o de Claude).
                for k, v in resultado["manoObra"].items():
                    data["manoObra"][k] = v
            if resultado and resultado.get("franquicia"):
                data["franquicia"] = resultado["franquicia"]
        except Exception:
            # Si el parser local falla, seguimos con lo que haya
            # devuelto Claude (o vacío) para no romper el informe.
            pass
