...
def procesar(plantilla_bytes, texto_peritacion="", excel_peritacion_bytes=None,
    ...
    if excel_peritacion_bytes:
        try:
            ...
            if resultado:
                ...
                if resultado.get("manoObra"):
                    manoObra_del_excel = resultado["manoObra"]
    ...
    # 5) Mano de obra y franquicia del Excel (merge, no pisar los
    #    *Valor unitarios que traiga la plantilla).
    if manoObra_del_excel:
        for k, v in manoObra_del_excel.items():
            data["manoObra"][k] = v
