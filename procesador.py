...
- PERITAJE DESDE EXCEL (GRILLA): si el texto incluye un bloque que
  ...
  * El nombre de cada pieza se arma combinando el SECTOR donde esta con
    ...
    delantero derecho".
  * Si la fila tiene un monto en la columna "Precio" del Excel, USALO como
    el precio de esa pieza (campo "precio" del item correspondiente en
    "danos"). Cada pieza con su precio individual. Solo numero entero,
    sin signo $ ni puntos de miles.
  * Si dos filas tienen la misma pieza (ej: dos "EMBLEMA" en la misma
    seccion), cada una se carga como item separado con SU PROPIO precio,
    incluso si el nombre se repite.
  * Del Excel tambien extraé los datos del vehiculo, asegurado, taller,
    mano de obra y observaciones si estan presentes.

- PRECIOS DE REPUESTOS desde COTIZACION pegada: el texto puede incluir,
  despues de una linea "=== COTIZACION DE REPUESTOS ===", una tabla o
  lista de precios (con columnas tipo Repuesto, Precio s/IVA, Precio
  c/IVA, etc.). Si esa cotizacion esta presente, TIENE PRIORIDAD sobre
  los precios del Excel:
  * Para cada pieza a CAMBIAR, asignale el precio de la cotizacion
    usando SIEMPRE la columna "Precio c/IVA" (el valor CON IVA).
  ...
  * Si una pieza a CAMBIAR no aparece en la cotizacion pero si en el
    Excel del peritaje, usá el precio del Excel.
  * Si no aparece en ninguno, su precio es 0.
- Tambien se acepta un precio escrito al lado de la pieza en el peritaje
  en prosa (ej: "Capot $180000").
- Si NO hay cotizacion y tampoco precios en el Excel, todas las piezas
  a CAMBIAR van con precio 0.
