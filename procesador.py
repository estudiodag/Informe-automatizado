...
- "varios" es un campo MONETARIO: contiene el SUBTOTAL EN PESOS (no la
  cantidad) de cualquier item de mano de obra que no encaje en
  pintura/chapa/mecanica/tapiceria. Ej: si el peritaje dice
  "Cristaleria 1 hora $100.000", "varios" = 100000. Si dice
  "carga de gas $50.000" y tambien "service $30.000", "varios" = 80000.
  Si no hay items extra, "varios" = 0.
- TOTAL DE REPUESTOS DIRECTO: si el peritaje incluye un total de
  repuestos ya calculado (ej: una fila "REPUESTOS $1.708.000" en el
  Excel, o "Total repuestos: $X"), guardalo en "totalRepuestos" como
  numero. Si no aparece, "totalRepuestos" = 0.
...
def parsear_texto(texto):
    ...
    data["totalRepuestos"] = a_numero(parsed.get("totalRepuestos", 0))
...
def completar_plantilla(plantilla_bytes, data):
    ...
    if len(hojas) > 2:
        ...
        # Si no hay desglose de precios por pieza pero el peritaje
        # trajo un total directo (ej: una fila "REPUESTOS $1.708.000"
        # en el Excel), usar ese valor.
        if total_repuestos == 0 and data.get("totalRepuestos"):
            total_repuestos = data["totalRepuestos"]
