...
- PERITAJE DESDE EXCEL (GRILLA): si el texto incluye un bloque que
  ...
  * El nombre de cada pieza se arma combinando el SECTOR donde esta con
    ...
    delantero derecho".
  * Si la fila tiene un monto en la columna "Precio" del Excel, USALO
    como el precio de esa pieza (campo "precio" del item correspondiente
    en "danos"). Cada pieza con su precio individual. Solo numero
    entero, sin signo $ ni puntos de miles.
...
def _copiar_imagenes(ws_origen, ws_destino):
    ...
    Lee los bytes directamente del BytesIO interno (img.ref) sin usar
    img._data(), porque _data() cierra el archivo y rompe el original.
    """
    try:
        ...
        for img in getattr(ws_origen, "_images", []):
            ...
            if hasattr(ref, "read"):
                try:
                    ref.seek(0)
                    datos = ref.read()
                    ref.seek(0)  # dejar el original como estaba
                except Exception:
                    datos = None
            if not datos:
                continue
            ...
            try:
                nueva.anchor = deepcopy(img.anchor)
            except Exception:
                pass
            ws_destino.add_image(nueva)
    except Exception:
        # Si algo falla, las imagenes no se copian pero el resto del
        # informe sale OK; preferimos eso a romper la generacion.
        pass
